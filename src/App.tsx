...
import { hasPickedLanguages, resetLanguagesPicked } from './lib/languages';
...
import {
  ...
  FiSliders, FiClock, FiMonitor, FiTerminal, FiGitBranch,
} from 'react-icons/fi';
...
function AiStatusButton() {
  ...
  return (
    <button title={cfg.label} onClick={handleClick}
      style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '0 10px', height: '100%', background: cfg.bg, border: 'none', borderLeft: '1px solid rgba(255,255,255,0.15)', color: '#fff', cursor: 'pointer', fontSize: 11, fontFamily: 'inherit', fontWeight: 600, letterSpacing: '0.04em', flexShrink: 0, whiteSpace: 'nowrap' }}
    ...
    >
      <span style={{ width: 7, height: 7, borderRadius: '50%', background: cfg.dot, display: 'inline-block', flexShrink: 0, boxShadow: cfg.dotGlow ? `0 0 6px ${cfg.dot}, 0 0 10px ${cfg.dot}88` : 'none' }} />
      {cfg.text}
    </button>
  );
}
...
function getDockRect(id: WinId, wsW: number, wsH: number, mode: Mode, sizes: DockSizes, visible?: Set<WinId>): WinRect | null {
  const { leftW, rightW, visualRightW, bottomH, terminalH } = sizes;
  const AB = ACTIVITY_BAR_W;

  const showTerm = visible ? visible.has('terminal') : false;
  const termH = showTerm ? terminalH : 0;
  const mainH = wsH - termH;
  const avW = wsW - AB;

  if (id === 'terminal') {
    const showFiles = !visible || visible.has('files');
    const effLeft = showFiles ? leftW : 0;
    return { x: AB + effLeft, y: mainH, w: avW - effLeft, h: terminalH };
  }
  ...
  if (mode === 'visual') {
    ...
    const cw = Math.max(160, avW - effLeft - effRight);

    if (id === 'files'      && showFiles) return { x: AB,                    y: 0,               w: leftW,        h: mainH };
    if (id === 'preview')                 return { x: AB + effLeft,           y: 0,               w: cw,           h: mainH - effBottom };
    if (id === 'timeline'   && showTL)    return { x: AB + effLeft,           y: mainH - effBottom, w: cw + effRight, h: bottomH };
    if (id === 'properties' && showProps) return { x: AB + effLeft + cw,      y: 0,               w: visualRightW, h: mainH - effBottom };
    return null;
  } else if (mode === 'code') {
    const showFiles = !visible || visible.has('files');
    const effLeft   = showFiles ? leftW : 0;
    if (id === 'files' && showFiles) return { x: AB,             y: 0, w: leftW,        h: mainH };
    if (id === 'code')               return { x: AB + effLeft,   y: 0, w: avW - effLeft, h: mainH };
    return null;
  } else {
    const showFiles   = !visible || visible.has('files');
    ...
    if (id === 'files'   && showFiles)   return { x: AB,                 y: 0, w: leftW,  h: mainH };
    if (id === 'code')                   return { x: AB + effLeft,        y: 0, w: cw,     h: mainH };
    if (id === 'preview' && showPreview) return { x: AB + effLeft + cw,   y: 0, w: rightW, h: mainH };
    return null;
  }
}
...
function getSnapSlots(wsW: number, wsH: number, mode: Mode, sizes: DockSizes, visible?: Set<WinId>): { id: WinId; rect: WinRect }[] {
  const ids: WinId[] =
    mode === 'visual' ? ['files', 'preview', 'timeline', 'properties', 'terminal'] :
    mode === 'code'   ? ['files', 'code', 'terminal'] :
                        ['files', 'code', 'preview', 'terminal'];
...
function floatingDefault(id: WinId, wsW: number, wsH: number): WinRect {
  const defaults: Record<WinId, WinRect> = {
    files:      { x: ACTIVITY_BAR_W + 20, y: 20,  w: 200, h: Math.min(520, wsH - 60) },
    code:       { x: ACTIVITY_BAR_W + 240, y: 20, w: Math.max(420, wsW * 0.4), h: Math.min(580, wsH - 60) },
    preview:    { x: Math.max(ACTIVITY_BAR_W + 240, wsW - Math.max(380, wsW * 0.38) - 20), y: 20, w: Math.max(380, wsW * 0.38), h: Math.min(500, wsH - 80) },
    properties: { x: Math.max(0, wsW - 280), y: 60, w: 265, h: Math.min(440, wsH - 100) },
    timeline:   { x: ACTIVITY_BAR_W + 20, y: Math.max(80, wsH - 200), w: Math.max(460, wsW * 0.6), h: 185 },
    terminal:   { x: ACTIVITY_BAR_W + 20, y: Math.max(80, wsH - 260), w: Math.max(500, wsW * 0.7), h: 220 },
  };
  return defaults[id];
}

const DEFAULT_DOCK: DockSizes = { leftW: 220, rightW: 420, visualRightW: 240, bottomH: 160, terminalH: 220 };
...
const LS_KEY = 'html-editor-win-layout-v8';
const LS_DOCK = 'html-editor-dock-sizes-v3';
...
function DockDivider({ orientation, pos, min, max, onChange }: DividerProps) {
  ...
  const onMd = (e: React.MouseEvent) => {
    ...
    const onMv = (ev: MouseEvent) => {
      onChange(Math.max(min, Math.min(max, startPos + (isV ? ev.clientX : ev.clientY) - startMouse)));
    };
    const onUp = () => {
  ...
  const bg = drag ? 'rgba(100,160,255,0.6)' : hov ? 'rgba(100,160,255,0.35)' : 'rgba(0,0,0,0.55)';
  return (
    <div onMouseDown={onMd} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{ ...style, background: bg, transition: 'background 0.15s' }}
    />
  );
}
      ...
      {slots.map(({ id, rect }) => {
        ...
        return (
          <div key={id} style={{
            ...
            borderRadius: 4, transition: 'all 0.1s', display: 'flex', alignItems: 'center', justifyContent: 'center',
          }}>
            {isHov && (
              <div style={{ background: 'rgba(100,160,255,0.85)', borderRadius: 6, padding: '6px 14px', fontSize: 12, fontWeight: 600, color: '#fff', boxShadow: '0 2px 8px rgba(0,0,0,0.4)' }}>
                Drop to dock here
              </div>
            )}
          </div>
...
/* ─────────────────────────────────────────
   ...
   ───────────────────────────────────────── */
type ActivityItem = 'explorer';

function ActivityBar({
  activeItem, onActivate, onToggleTerminal, terminalVisible, ports,
}: {
  ...
  const items: { id: ActivityItem; icon: React.ReactNode; title: string }[] = [
    { id: 'explorer', icon: <FiFolder size={22} />, title: 'Explorer (Ctrl+E)' },
  ];

  return (
    <div style={{
      position: 'absolute', left: 0, top: 0, width: ACTIVITY_BAR_W, height: '100%',
      background: '#333333', display: 'flex', flexDirection: 'column',
      alignItems: 'center', padding: '4px 0', gap: 0, zIndex: 600,
      ...
      {items.map(item => (
        <button
          ...
          style={{
            ...
            borderLeft: `2px solid ${activeItem === item.id ? '#fff' : 'transparent'}`,
          }}
          onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = '#fff'; }}
          onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = activeItem === item.id ? '#fff' : 'rgba(255,255,255,0.4)'; }}
        >
          {item.icon}
        </button>
      ))}

      {/* Port badges in activity bar */}
      {ports.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, marginTop: 4 }}>
          {ports.map(p => (
            <a key={p.port} href={p.url} target="_blank" rel="noreferrer" title={`Port ${p.port}: ${p.url}`}
              style={{ fontSize: 9, color: '#4ec9b0', fontWeight: 700, textDecoration: 'none', background: 'rgba(78,201,176,0.15)', borderRadius: 3, padding: '1px 4px' }}>
              :{p.port}
            </a>
          ))}
        </div>
      ...
      <button
        ...
        style={{
          ...
          color: terminalVisible ? '#4ec9b0' : 'rgba(255,255,255,0.4)',
          borderLeft: `2px solid ${terminalVisible ? '#4ec9b0' : 'transparent'}`,
          transition: 'color 0.15s',
        }}
        onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = '#fff'; }}
        onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = terminalVisible ? '#4ec9b0' : 'rgba(255,255,255,0.4)'; }}
      >
        <FiTerminal size={22} />
      </button>
...
/* ─────────────────────────────────────────
   Mobile App
   ───────────────────────────────────────── */
type MobileTab = 'code' | 'preview' | 'files' | 'props' | 'terminal';
...
function MobileApp() {
  ...
  const TABS: { id: MobileTab; icon: React.ReactNode; label: string }[] = [
    ...
    { id: 'terminal', icon: <FiTerminal size={18} />, label: 'Terminal' },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', width: '100vw', background: bg, color: '#ccc', fontFamily: "'Inter', -apple-system, sans-serif", overflow: 'hidden' }}>
      <div style={{ display: 'flex', alignItems: 'center', height: 46, flexShrink: 0, background: '#323233', borderBottom: `1px solid ${border}`, padding: '0 12px', gap: 10 }}>
        <div style={{ width: 20, height: 20, borderRadius: 4, background: '#007acc', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 900, color: '#fff' }}>VS</div>
        <span style={{ fontSize: 13, fontWeight: 600, color: '#ccc', flex: 1 }}>Code Editor</span>
      ...
      <div style={{ flex: 1, overflow: 'hidden', position: 'relative' }}>
        ...
        <div style={{ display: tab === 'terminal' ? 'flex' : 'none', flexDirection: 'column', height: '100%' }}><TerminalPanel /></div>
        <div style={{ display: tab === 'props' ? 'flex' : 'none', flexDirection: 'column', height: '100%', overflowY: 'auto' }}><PropertiesPanel hideHeader /></div>
      </div>
      <div style={{ display: 'flex', height: 58, flexShrink: 0, background: bar, borderTop: `1px solid ${border}`, paddingBottom: 'env(safe-area-inset-bottom)' }}>
        {TABS.map(t => {
          ...
          return (
            <button key={t.id} onClick={() => setTab(t.id)} style={{
              ...
              color: active ? accent : '#666', transition: 'all 0.15s',
            }}>
              {t.icon}
...
/* ─────────────────────────────────────────
   ...
   ───────────────────────────────────────── */
type GateStage = 'language' | 'booting' | 'ready';
...
function EditorGate() {
  ...
  const [stage, setStage] = useState<GateStage>(() =>
    hasPickedLanguages() ? 'booting' : 'language'
  );
  const [bootDone, setBootDone] = useState(false);
  const [bootFailed, setBootFailed] = useState(false);
  const startedRef = useRef(false);

  // Kick off the full boot sequence as soon as we enter the booting stage.
  useEffect(() => {
    if (stage !== 'booting' || startedRef.current) return;
    startedRef.current = true;
    (async () => {
      try {
        wcManager.log('info', 'Starting workspace setup…');
        await wcManager.boot();
        const files = useEditorStore.getState().files;
        if (files.length > 0) await wcManager.mountFiles(files);
        await wcManager.maybeInstallDeps();
        wcManager.log('ok', 'Workspace ready — opening editor.');
        setBootDone(true);
      } catch (e: any) {
        wcManager.log('err', `Setup failed: ${e?.message || e}`);
        setBootFailed(true);
        setBootDone(true);
      }
    })();
  ...
  const retry = useCallback(() => {
    ...
    setBootDone(false);
    setBootFailed(false);
    // Re-enter booting (effect will re-run because startedRef is reset).
    setStage('language');
    setTimeout(() => setStage('booting'), 0);
  }, []);

  void resetLanguagesPicked;

  if (stage === 'language') {
    return <LanguageSelect onContinue={() => setStage('booting')} />;
  }
  if (stage === 'booting') {
    return (
      <BootProgress
...
function DesktopApp() {
  ...
  // WebContainer ↔ File sync
  useWebContainerSync();

  const wsRef = useRef<HTMLDivElement>(null);
  const [wsSize, setWsSize] = useState({ w: 1200, h: 660 });
  const [ports, setPorts] = useState<PortInfo[]>(wcManager.ports);

  useEffect(() => {
    const unsub = wcManager.onPorts(setPorts);
    return unsub;
  }, []);
  ...
  const [dragState, setDragState] = useState<{
    winId: WinId; pos: { x: number; y: number }; hovered: WinId | null;
  } | null>(null);
  ...
  const toggleTerminal = useCallback(() => {
    setWins(ws => ws.map(w => {
      if (w.id !== 'terminal') return w;
      const newVisible = !w.visible;
      return { ...w, visible: newVisible, docked: true, minimized: false, zIndex: newVisible ? nextZ() : w.zIndex };
    }));
  }, []);

  const handleActivityClick = useCallback((_item: ActivityItem) => {
    setActivityItem(prev => {
      if (prev === 'explorer') {
        setWins(ws => ws.map(w => w.id === 'files' ? { ...w, visible: false } : w));
        return null;
      }
      setWins(ws => ws.map(w => w.id === 'files' ? { ...w, visible: true, docked: true, minimized: false, zIndex: nextZ() } : w));
      return 'explorer';
    });
  }, []);
  ...
  const applyModePreset = useCallback((m: Mode) => {
    ...
    setWins(ws => ws.map(w => {
      if (w.id === 'terminal') return w;
      return { ...w, visible: visible.has(w.id), docked: visible.has(w.id), minimized: false, zIndex: visible.has(w.id) ? nextZ() : w.zIndex };
    }));
    if (m === 'code' || m === 'split') setActivityItem('explorer');
  }, [setMode]);
  ...
  const visibleDockedSet = new Set<WinId>(wins.filter(w => w.visible && w.docked).map(w => w.id));
  ...
  const onDragEnd = useCallback((winId: WinId, cx: number, cy: number) => {
    setDragState(prev => {
      if (prev?.hovered) {
        ...
        setTimeout(() => {
          setWins(ws => ws.map(w => w.id === winId ? { ...w, docked: true, zIndex: nextZ() } : w));
        }, 0);
      }
      return null;
    });
  ...
  }, [files, applyModePreset, toggleTerminal]);
  ...
  const terminalVisible = winMap.terminal?.visible && winMap.terminal?.docked;
  ...
  function winContent(id: WinId) {
    switch (id) {
      ...
      case 'terminal':   return <TerminalPanel onClose={() => updateWin('terminal', { visible: false })} />;
    }
  }

  const mainH = wsSize.h - (terminalVisible ? dockSizes.terminalH : 0);

  const showLeftDiv  = winMap.files.visible && winMap.files.docked &&
                       (['code', 'preview'].some(id => winMap[id as WinId]?.visible && winMap[id as WinId]?.docked));
  ...
  const showBotDiv   = mode === 'visual' && winMap.timeline.visible && winMap.timeline.docked && winMap.preview.visible && winMap.preview.docked;
  const showTermDiv  = terminalVisible;
  ...
  const winTitle: Record<WinId, string> = {
    files: 'Explorer', code: 'Code Editor',
    preview: mode === 'visual' ? 'Visual Editor' : 'Preview',
    properties: 'Properties', timeline: 'Timeline', terminal: 'Terminal',
  };

  const shortLabels: Record<WinId, string> = {
    files: 'Explorer', code: 'Code', preview: 'Preview',
    properties: 'Props', timeline: 'Timeline', terminal: 'Terminal',
  };
  ...
  return (
    ...
    >
      ...
      {/* ── Title Bar ── */}
      <div style={{ display: 'flex', alignItems: 'center', height: 30, flexShrink: 0, background: '#323233', borderBottom: '1px solid #252526', position: 'relative', zIndex: 9999 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '0 12px', flexShrink: 0 }}>
          <div style={{ width: 18, height: 18, borderRadius: 3, background: '#e34c26', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 900, color: '#fff', letterSpacing: '-0.02em', boxShadow: '0 0 8px rgba(227,76,38,0.35)' }}>H</div>
          <span style={{ fontSize: 12, fontWeight: 600, color: '#ccc' }}>HTML Editor</span>
        </div>
        <MenuBar wins={wins} onToggleWin={toggleWin} onOpenWin={openWin} onResetLayout={resetLayout} onApplyModePreset={applyModePreset} />
        <div style={{ flex: 1 }} />
        <span style={{ fontSize: 11, color: '#555', padding: '0 12px' }}>{activeFileName}</span>
      </div>

      {/* ── Toolbar ── */}
      <div style={{ display: 'flex', alignItems: 'center', height: 36, flexShrink: 0, background: '#2d2d2d', borderBottom: '1px solid #1e1e1e', padding: `0 10px 0 ${ACTIVITY_BAR_W + 10}px`, gap: 4, position: 'relative', zIndex: 9998 }}>
        ...
        {([['split', 'Split', FiLayout, 'Ctrl+3'], ['code', 'Code', FiCode, 'Ctrl+1'], ['visual', 'Visual', FiEye, 'Ctrl+2']] as const).map(([m, label, Icon, sc]) => (
          <button key={m} title={`${label} layout (${sc})`} onClick={() => applyModePreset(m)}
            style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '4px 12px', borderRadius: 4, cursor: 'pointer', fontSize: 12, fontWeight: 500, fontFamily: 'inherit', background: mode === m ? 'rgba(0,122,204,0.2)' : 'transparent', border: `1px solid ${mode === m ? 'rgba(0,122,204,0.6)' : 'transparent'}`, color: mode === m ? '#4fc1ff' : '#888' }}>
            <Icon size={13} />{label}
          </button>
        ))}
        ...
        {/* Port selector in toolbar */}
        {ports.length > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            <span style={{ fontSize: 11, color: '#4ec9b0' }}>Ports:</span>
            {ports.map(p => (
              <a key={p.port} href={p.url} target="_blank" rel="noreferrer"
                style={{ padding: '2px 8px', fontSize: 11, borderRadius: 3, background: 'rgba(78,201,176,0.12)', border: '1px solid rgba(78,201,176,0.35)', color: '#4ec9b0', textDecoration: 'none', cursor: 'pointer' }}
                title={p.url}
              >:{p.port}</a>
            ))}
          </div>
        ...
        {(['files', 'code', 'preview', 'properties', 'timeline', 'terminal'] as WinId[]).map(id => {
          const w = winMap[id];
          return (
            <button key={id} onClick={() => id === 'terminal' ? toggleTerminal() : toggleWin(id)}
              title={w.docked ? `${shortLabels[id]} (docked)` : `${shortLabels[id]} ${w.visible ? '(floating)' : '(hidden)'}`}
              style={{ padding: '2px 8px', fontSize: 11, borderRadius: 3, cursor: 'pointer', fontFamily: 'inherit', background: w.visible ? (w.docked ? 'rgba(0,122,204,0.12)' : 'rgba(229,164,90,0.12)') : '#1a1a1a', border: `1px solid ${w.visible ? (w.docked ? 'rgba(0,122,204,0.5)' : 'rgba(229,164,90,0.4)') : '#3e3e3e'}`, color: w.visible ? (w.docked ? '#4fc1ff' : '#e5a45a') : '#666' }}
            >
      ...
      {/* ── Workspace ── */}
      <div ref={wsRef} style={{ flex: 1, position: 'relative', overflow: 'hidden', background: '#1e1e1e' }}>
        ...
        {/* Snap zone overlay */}
        <SnapZonesOverlay slots={snapSlots} dragPos={dragState?.pos ?? null} hoveredSlot={dragState?.hovered ?? null} />

        {/* Dock dividers */}
        {showLeftDiv && (
          <DockDivider orientation="vertical" pos={ACTIVITY_BAR_W + dockSizes.leftW} min={ACTIVITY_BAR_W + 100} max={Math.max(ACTIVITY_BAR_W + 121, wsSize.w - (mode === 'visual' ? dockSizes.visualRightW : dockSizes.rightW) - 160)}
            wsW={wsSize.w} wsH={wsSize.h}
            onChange={v => setDockSizes(s => ({ ...s, leftW: Math.max(100, v - ACTIVITY_BAR_W) }))}
          />
        )}
        {showRightDiv && mode !== 'visual' && (
          <DockDivider orientation="vertical" pos={wsSize.w - dockSizes.rightW} min={Math.max(121, ACTIVITY_BAR_W + dockSizes.leftW + 160)} max={wsSize.w - 160}
            wsW={wsSize.w} wsH={wsSize.h}
            onChange={v => setDockSizes(s => ({ ...s, rightW: Math.max(160, wsSize.w - v) }))}
          />
        )}
        {showBotDiv && (
          <DockDivider orientation="horizontal" pos={mainH - dockSizes.bottomH} min={100} max={mainH - 80}
            wsW={wsSize.w} wsH={wsSize.h}
            onChange={v => setDockSizes(s => ({ ...s, bottomH: Math.max(60, mainH - v) }))}
          />
        )}
        {mode === 'visual' && winMap.properties.visible && winMap.properties.docked && (
          <DockDivider orientation="vertical" pos={wsSize.w - dockSizes.visualRightW} min={Math.max(121, ACTIVITY_BAR_W + dockSizes.leftW + 160)} max={wsSize.w - 160}
            wsW={wsSize.w} wsH={wsSize.h}
            onChange={v => setDockSizes(s => ({ ...s, visualRightW: Math.max(160, wsSize.w - v) }))}
          />
        )}
        {showTermDiv && (
          <DockDivider orientation="horizontal" pos={mainH} min={Math.max(120, wsSize.h - 600)} max={wsSize.h - 80}
            wsW={wsSize.w} wsH={wsSize.h}
            onChange={v => setDockSizes(s => ({ ...s, terminalH: Math.max(80, wsSize.h - v) }))}
          />
        )}
      ...
      {/* ── Status Bar ── */}
      <div style={{ display: 'flex', alignItems: 'center', height: 24, flexShrink: 0, background: '#007acc', padding: '0 0 0 0', gap: 0, fontSize: 11, color: 'rgba(255,255,255,0.9)', zIndex: 200 }}>
        <div style={{ display: 'flex', alignItems: 'center', height: '100%', background: 'rgba(0,0,0,0.2)', padding: '0 12px', gap: 6, flexShrink: 0, borderRight: '1px solid rgba(255,255,255,0.15)' }}>
          <FiGitBranch size={12} />
          <span style={{ fontWeight: 600 }}>main</span>
        </div>
        <span style={{ padding: '0 12px', opacity: 0.85 }}>{activeFileName || 'No file'}</span>
        {ports.length > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '0 10px', borderLeft: '1px solid rgba(255,255,255,0.15)' }}>
            {ports.map(p => (
              <a key={p.port} href={p.url} target="_blank" rel="noreferrer"
                style={{ color: '#fff', opacity: 0.95, textDecoration: 'none', fontSize: 11 }}
                title={`Open ${p.url}`}
              >🌐 :{p.port}</a>
            ))}
          </div>
        ...
        <div style={{ flex: 1 }} />
        <span style={{ padding: '0 10px', borderLeft: '1px solid rgba(255,255,255,0.15)', opacity: 0.8 }}>{files.length} files</span>
        <AiStatusButton />
      </div>
